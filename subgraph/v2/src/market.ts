...
import { Market, MarketEvent, Position } from "../../generated/v2/schema";
import {
...
} from "../../generated/v2/ReferenceLendingMarket/ReferenceLendingMarket";
...
function marketFor(event: ethereum.Event): Market {
  ...
  if (market === null) {
    ...
    market.contractVersion = "v2";
    market.stateVersion = contract.stateVersion();
...
// Apply the emitted version, never infer it from event counts or cached state.
// A constructor read sees end-of-block state; ordered version events then bring
// it through that block's mutations to the same final version.
export function handleStateUpdated(event: MarketStateUpdated): void {
  const market = marketFor(event);
  market.stateVersion = event.params.stateVersion;
  market.updatedAtBlock = event.block.number;
  market.save();
  logEvent(event, market, "STATE_VERSION");
}
